...
def getWalle():
    ...
    if request.method == 'GET':
        agentPositions = [{"id": str(a.unique_id), "x": x, "y": 1, "z": z}
                          for (a, x, z) in AlmacenModel.grid.coord_iter()
                          if isinstance(a, Walle)]
...
def getObstacles():
    ...
    if request.method == 'GET':
        cajaPositions = [{"id": str(a.unique_id), "x": x, "y": 1, "z": z}
                         for (a, x, z) in AlmacenModel.grid.coord_iter()
                         if isinstance(a, Cajas)]
...
def getRepisas():
    ...
    if request.method == 'GET':
        repisaPositions = [{"id": str(a.unique_id), "x": x, "y": 1, "z": z}
                           for (a, x, z) in AlmacenModel.grid.coord_iter()
                           if isinstance(a, Cajas)]
